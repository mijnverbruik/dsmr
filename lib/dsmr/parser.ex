defmodule DSMR.Parser do
  ...
  defp attrs_from_object({[0, 0, 1, 0, 0], value}, %Telegram{} = telegram) do
    %{telegram | measured_at: value}
  end

  defp attrs_from_object({[0, 0, 96, 1, 1], value}, %Telegram{} = telegram) do
    %{telegram | equipment_id: value}
  end

  defp attrs_from_object({[1, 0, 1, 8, 1], value}, %Telegram{} = telegram) do
    %{telegram | electricity_delivered_1: value}
  end

  defp attrs_from_object({[1, 0, 1, 8, 2], value}, %Telegram{} = telegram) do
    %{telegram | electricity_delivered_2: value}
  end

  defp attrs_from_object({[1, 0, 2, 8, 1], value}, %Telegram{} = telegram) do
    %{telegram | electricity_returned_1: value}
  end

  defp attrs_from_object({[1, 0, 2, 8, 2], value}, %Telegram{} = telegram) do
    %{telegram | electricity_returned_2: value}
  end

  defp attrs_from_object({[0, 0, 96, 14, 0], value}, %Telegram{} = telegram) do
    %{telegram | electricity_tariff_indicator: value}
  end

  defp attrs_from_object({[1, 0, 1, 7, 0], value}, %Telegram{} = telegram) do
    %{telegram | electricity_currently_delivered: value}
  end

  defp attrs_from_object({[1, 0, 2, 7, 0], value}, %Telegram{} = telegram) do
    %{telegram | electricity_currently_returned: value}
  end

  defp attrs_from_object({[0, 0, 96, 7, 21], value}, %Telegram{} = telegram) do
    %{telegram | power_failures_count: value}
  end

  defp attrs_from_object({[0, 0, 96, 7, 9], value}, %Telegram{} = telegram) do
    %{telegram | power_failures_long_count: value}
  end

  defp attrs_from_object({[1, 0, 99, 97, 0], value}, %Telegram{} = telegram) do
  ...
  defp attrs_from_object({[1, 0, 32, 32, 0], value}, %Telegram{} = telegram) do
    %{telegram | voltage_sags_l1_count: value}
  end

  defp attrs_from_object({[1, 0, 52, 32, 0], value}, %Telegram{} = telegram) do
    %{telegram | voltage_sags_l2_count: value}
  end

  defp attrs_from_object({[1, 0, 72, 32, 0], value}, %Telegram{} = telegram) do
    %{telegram | voltage_sags_l3_count: value}
  end

  defp attrs_from_object({[1, 0, 32, 36, 0], value}, %Telegram{} = telegram) do
    %{telegram | voltage_swells_l1_count: value}
  end

  defp attrs_from_object({[1, 0, 52, 36, 0], value}, %Telegram{} = telegram) do
    %{telegram | voltage_swells_l2_count: value}
  end

  defp attrs_from_object({[1, 0, 72, 36, 0], value}, %Telegram{} = telegram) do
    %{telegram | voltage_swells_l3_count: value}
  end

  defp attrs_from_object({[0, 0, 96, 13, 1], value}, %Telegram{} = telegram) do
    %{telegram | text_message_codes: value}
  end

  defp attrs_from_object({[0, 0, 96, 13, 0], value}, %Telegram{} = telegram) do
    %{telegram | text_message: value}
  end

  defp attrs_from_object({[1, 0, 31, 7, 0], value}, %Telegram{} = telegram) do
    %{telegram | phase_power_current_l1: value}
  end

  defp attrs_from_object({[1, 0, 51, 7, 0], value}, %Telegram{} = telegram) do
    %{telegram | phase_power_current_l2: value}
  end

  defp attrs_from_object({[1, 0, 71, 7, 0], value}, %Telegram{} = telegram) do
    %{telegram | phase_power_current_l3: value}
  end

  defp attrs_from_object({[1, 0, 21, 7, 0], value}, %Telegram{} = telegram) do
    %{telegram | currently_delivered_l1: value}
  end

  defp attrs_from_object({[1, 0, 41, 7, 0], value}, %Telegram{} = telegram) do
    %{telegram | currently_delivered_l2: value}
  end

  defp attrs_from_object({[1, 0, 61, 7, 0], value}, %Telegram{} = telegram) do
    %{telegram | currently_delivered_l3: value}
  end

  defp attrs_from_object({[1, 0, 22, 7, 0], value}, %Telegram{} = telegram) do
    %{telegram | currently_returned_l1: value}
  end

  defp attrs_from_object({[1, 0, 42, 7, 0], value}, %Telegram{} = telegram) do
    %{telegram | currently_returned_l2: value}
  end

  defp attrs_from_object({[1, 0, 62, 7, 0], value}, %Telegram{} = telegram) do
    %{telegram | currently_returned_l3: value}
  end

  defp attrs_from_object({[0, _channel, 24, 1, 0], value}, %MBusDevice{} = mbus_device) do
    %{mbus_device | device_type: value}
  end

  defp attrs_from_object({[0, _channel, 96, 1, 0], value}, %MBusDevice{} = mbus_device) do
    %{mbus_device | equipment_id: value}
  end

  defp attrs_from_object({[0, _channel, 24, 2, 1], value}, %MBusDevice{} = mbus_device) do
